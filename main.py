# -*- coding: utf-8 -*-
"""kaggle_comp_final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1jk2kIgfClqQQoqc3_V7deUEVlJ02DjXk
...
groups = [df[df['Дом'] == cat]['usd_price'] for cat in df['Дом'].unique()]
...
p-value маленький (< 0.05) — различия между группами статистически значимы, признак, скорее всего, полезен для модели.
Однако, epsilon-squared показывает слабое различие, согласно градации:

*   <0.01 — пренебрежимо мало
*   0.01–0.06 — слабый эффект
*   0.06–0.14 — средний
*   0.14 — сильный
...
rfr = RandomForestRegressor(n_estimators=443, max_depth=21, min_samples_split=2, min_samples_leaf=1, max_features = 0.3183850552445241, bootstrap=False,
...
print("Test MAPE: {:.4f}".format(mean_absolute_percentage_error(y_test, y_pred)))
...
# Контурный график для пары параметров
...
feature_names = X_train.columns  # или model.feature_names_

# сортировка по возрастанию (для горизонтального barh)
